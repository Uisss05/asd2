...
// Route untuk meng-upload quotes dan foto
router.post("/", upload.single("foto"), (req, res) => {
  const { judul, hook, isiQuotes, footnote, penulis } = req.body;
  ...
  const query = `
    INSERT INTO quotes (judul, isiQuotes, footnote, penulis, foto, hook, slug, created_at) 
    VALUES (?, ?, ?, ?, ?, ?, ?, NOW())
  `;

  db.query(query, [judul, isiQuotes, footnote, penulis, fotoPath, hook, slug], (error, results) => {
    if (error) {
      console.error("Terjadi kesalahan:", error);
      res.status(500).json({ message: "Gagal menyimpan quotes", error });
    } else {
      res.status(200).json({ message: "Quotes berhasil disimpan!", quotesId: results.insertId });
    }
  });
});
